...
// CONTROLLER STUDENT
export const getProfileStudent = async (c: Context) => {
    try {
        const user = c.get("user")
        const result = await service.getProfileStudent(user)
        return c.json(response.successResponse(result))
    } catch (err: any) {
        if (err instanceof ZodError) {
            return c.json({
...
export const updateProfileStudent = async (c: Context) => {
    try {
        const user = c.get("user")
        const body = await c.req.json()
        const data = updateProfileSchema.parse(body)
        const result = await service.updateProfileStudent(user, data)
        return c.json(response.successResponse(result))
    } catch (err: any) {
        if (err instanceof ZodError) {
            return c.json({
